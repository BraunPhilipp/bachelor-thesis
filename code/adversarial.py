        ...
        if method == "quadratic":
            ...
            for epsilon in epsilon_range:
                ...
                for v in value:
                    ...
                    fY = sess.run([output], feed_dict={input: np.array([v]), pkeep: 1.0})
                    w = None
                    x = v.copy()
                    ...
                    for iter in range(num_iterations):
                        ...
                        if norm == "l2":
                            dX = sess.run([grad], feed_dict={input: np.array([x]), pkeep: 1.0})
                            ...
                            for hh in range(H):
                                for ww in range(W):
                                    ...
                                    if (np.sum(Jvec.flatten()**2) >= Jvec_norm) and ([hh,ww] not in used_pixels):
                                        h_opt = hh
                                        w_opt = ww
                                        rho_opt = np.zeros(D) + rho[idx[-1]] # rho
                            ...
                            for hh in range(H):
                                for ww in range(W):
                                    for dd in range(D):
                                        norms[hh,ww,dd] = np.sum(dX[:,hh,ww,dd].flatten()**2)
                                        idx_mtx[:, hh, ww, dd] = np.array([hh, ww, dd])
                                    idx = np.argsort(norms[hh,ww,:])
                                    rho = np.zeros(D)
                                    rho[idx[0]] = random.choice([-1, 1])
                                    Jvec = dX[:,hh,ww,idx[0]]
                                    if len(idx)>1:
                                        for kk in range(len(idx) - 1):
                                            Jk = dX[:, hh, ww, idx[kk + 1]]
                    ...
                    # Images
                    fooledY = sess.run([output], feed_dict={input: np.array([x]), pkeep: 1.0})
                    ...
                    if params["description"] == "superresolution":
                        X_org, X_new, fY, fooledY = helpers.adjust_images(X_org, X_new, fY, fooledY, 200)
            ...
            for epsilon in epsilon_range:
                ...
                for x in value:
                    ...
                    if params["description"] == "autoencoder":
                        ...
                        Y_ = np.array( sess.run([output], feed_dict={input: np.array([x_org]), pkeep: 1.0}) )
                        true_Y = tf.placeholder(tf.float32, name="true_Y", shape=Y_.shape)
                        sess.run(true_Y, feed_dict={true_Y: Y_, pkeep: 1.0})
                    ...
                    for iter in range(num_iterations):
                        print('({}) {}-{}: Image {}/{} (iteration {}/{})'.format(str(epsilon),
                            method, norm, str(ii_imag), str(len(value)), str(iter + 1), num_iterations))
                        ...
                        dL = sess.run([grad], feed_dict={input: fX, pkeep: 1.0, true_Y: Y_})
                        # print(np.array(dL).tolist())
                    ...
                    fY = sess.run([output], feed_dict={input: np.array([x_org]), true_Y: Y_, pkeep: 1.0})
                    fooledY = sess.run([output], feed_dict={input: np.array(x), true_Y: Y_, pkeep: 1.0})
            ...
            for epsilon in epsilon_range:
                ...
                for v in value:
                    ...
                    _w = _v + eta
                    _fooledY = sess.run([output], feed_dict={input: np.array([_w]), pkeep: 1.0})
...
def main():
    ...
    adv_dict = {
        ...
        'lin-linf-1': 3, 'lin-linf-10': 4, 'lin-linf-20': 5,
        'lin-l2-1': 6, 'lin-l2-10': 7, 'lin-l2-20': 8,
        'quad-linf-1': 9, 'quad-linf-10': 10,
        'quad-l2-1': 11, 'quad-l2-10': 12,
        'lin-1': 13, 'lin-10':14,
        'rand-1': 15, 'rand-10': 16,
        'pixel': 17, 'pixel-2': 18, 'pixel-3': 19, 'pixel-100': 20
    }
    img_mtx = {}
    ...
    if params["norm"] == "l2":
        ...
        if params["dataset"] == "mnist" or params["dataset"] == "cifar":
            ...
            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                ["rand-l2", "lin-l2-1", "lin-l2-10", "lin-l2-20", "quad-l2-1"], "l2", img_size=params["image_dims"])
        else:
            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                ["rand-l2", "lin-l2-1", "lin-l2-10", "lin-l2-20"], "l2", img_size=params["image_dims"])
        ...
        adv_mtx[:, adv_dict['lin-linf-1']], img_mtx['lin-linf-1'] = \
            adv_noise(test_image, params, epsilon_range, "linear", "linf", False, 1)
        adv_mtx[:, adv_dict['lin-linf-10']], img_mtx['lin-linf-10'] = \
            adv_noise(test_image, params, epsilon_range, "linear", "linf", False, 10)
        adv_mtx[:, adv_dict['lin-linf-20']], img_mtx['lin-linf-20'] = \
            adv_noise(test_image, params, epsilon_range, "linear", "linf", False, 20)

        helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
            ["rand-linf", "lin-linf-1", "lin-linf-10", "lin-linf-20"], "linf", img_size=params["image_dims"])
        ...
        adv_mtx[:, adv_dict['rand-1']], img_mtx['rand-1'] = \
            adv_noise(test_image, params, epsilon_range, "rand", "pixel", False, 1)

        adv_mtx[:, adv_dict['rand-100']], img_mtx['rand-100'] = \
            adv_noise(test_image, params, epsilon_range, "rand", "pixel", False, 100)

        adv_mtx[:, adv_dict['lin-1']], img_mtx['lin-1'] = \
            adv_noise(test_image, params, epsilon_range, "linear", "pixel", False, 1)

        adv_mtx[:, adv_dict['lin-100']], img_mtx['lin-100'] = \
           adv_noise(test_image, params, epsilon_range, "linear", "pixel", False, 100)

        if params["dataset"] == "mnist" or params["dataset"] == "cifar":
            adv_mtx[:, adv_dict['pixel']], img_mtx['pixel'] = \
                adv_noise(test_image, params, epsilon_range, "quadratic", "pixel", False, 1)

            adv_mtx[:, adv_dict['pixel-100']], img_mtx['pixel-100'] = \
                adv_noise(test_image, params, epsilon_range, "quadratic", "pixel", False, 100)

            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                    ['rand-1','rand-100','lin-1','lin-100','pixel','pixel-100'], "pixel", img_size=params["image_dims"])
        else:
            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                    ['rand-1','rand-100','lin-1','lin-100'], "pixel", img_size=params["image_dims"])

    elif params["norm"] == "pixel-2":
        # Epsilon
        epsilon_range = epsilon_range = np.array([0.1, 0.2, 0.3, 0.5, 0.7])
        adv_mtx = np.zeros([len(epsilon_range), len(adv_dict)])
        ...
        adv_mtx[:, adv_dict['rand-10']], img_mtx['rand-10'] = \
            adv_noise(test_image, params, epsilon_range, "rand", "pixel", False, 10)

        adv_mtx[:, adv_dict['lin-10']], img_mtx['lin-10'] = \
            adv_noise(test_image, params, epsilon_range, "linear", "pixel", False, 10)
        ...
        if params["dataset"] == "mnist" or params["dataset"] == "cifar":
            ...
            adv_mtx[:, adv_dict['pixel']], img_mtx['pixel'] = \
                adv_noise(test_image, params, epsilon_range, "quadratic", "pixel", False, 10)

            adv_mtx[:, adv_dict['pixel-2']], img_mtx['pixel-2'] = \
                adv_noise(test_image, params, epsilon_range, "quadratic", "pixel-2", False, 10)

            adv_mtx[:, adv_dict['pixel-3']], img_mtx['pixel-3'] = \
                adv_noise(test_image, params, epsilon_range, "quadratic", "pixel-3", False, 10)

            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                    ['rand-10','lin-10','pixel','pixel-2','pixel-3'], "pixel-2", img_size=params["image_dims"])
        else:
            helpers.save_image_fig(img_mtx, params, adv_mtx[:, adv_dict['psnr_input']],
                    ['rand-10','lin-10'], "pixel-2", img_size=params["image_dims"])


    np.savetxt(params['output_dir'] + '/summary/' + params['model'] + '_psnr_summary_' + params['dataset'] + '_' + params["norm"] + '.csv', adv_mtx, delimiter=";")

    if params["norm"] == "pixel" or params["norm"] == "pixel-2":
        helpers.save_psnr_fig_2(adv_mtx, './results/images/' + params['figs_dir'] + '/' + params['model'] +'_fig_' + params['dataset'] + '_' + params["norm"],
            adv_dict, legend=True)
    else:
        helpers.save_psnr_fig(adv_mtx, './results/images/' + params['figs_dir'] + '/' + params['model'] +'_fig_' + params['dataset'] + '_' + params["norm"],
            adv_dict, legend=True)
